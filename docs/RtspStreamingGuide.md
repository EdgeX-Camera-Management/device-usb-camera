...
| ----------- | ----------- |----------- |
| `build-essential` | deveopment tools for compiling software in Linux | compile the various services|
| `ffmpeg`     | Multimedia framework | play video streams from usb camera |
...
| `v4l-utils`   | used to handle media devices | determine the video stream path |
